...
registerBlockType('custom/circle-progress-bar', {
    ...
    edit({ attributes, setAttributes }) {
        const {
            percentage, size, strokeColor, bgColor, text,
            animationSpeed, useShadow, useGradient,
            ...
            <>
                <InspectorControls>
                    <PanelBody title="Settings">
                        ...
                        />
                        {renderColorPicker('Stroke Color', strokeColor, val => setAttributes({ strokeColor: val }))}
                        {renderColorPicker('Background Color', bgColor, val => setAttributes({ bgColor: val }))}
                        {renderColorPicker('Font Color', fontColor, val => setAttributes({ fontColor: val }))}
                        <SelectControl
                            label="Use Gradient?"
                            value={useGradient ? 'yes' : 'no'}
                            options={[{ label: 'No', value: 'no' }, { label: 'Yes', value: 'yes' }]}
                            onChange={(val) => setAttributes({ useGradient: val === 'yes' })}
                        />
                        <SelectControl
                            label="Use Shadow?"
                            value={useShadow ? 'yes' : 'no'}
                            options={[{ label: 'No', value: 'no' }, { label: 'Yes', value: 'yes' }]}
                            onChange={(val) => setAttributes({ useShadow: val === 'yes' })}
                        />
                    </PanelBody>
                </InspectorControls>

                <svg height={size} width={size} style={{ display: 'block', margin: 'auto' }}>
                    {useGradient && (
                        <defs>
                    ...
                    <circle
                        ...
                        style={{
                            ...
                            filter: useShadow ? 'drop-shadow(0 0 5px rgba(0,0,0,0.5))' : 'none'
                        }}
                    />
                    <text
                        ref={textRef}
    ...
    save({ attributes }) {
        const {
            percentage, size, strokeColor, bgColor, text,
            useShadow, useGradient, strokeWidth, fontSize, fontColor
        } = attributes;
